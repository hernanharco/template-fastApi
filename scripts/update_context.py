...
class ContextAnalyzer:
    ...
    def _analyze_model_class(self, class_node: ast.ClassDef, file_path: Path) -> ModelInfo:
    ...
    def _analyze_schema_class(self, class_node: ast.ClassDef, file_path: Path) -> SchemaInfo:
    ...
    def _extract_endpoints_from_content(self, content: str, file_path: Path) -> None:
        ...
        decorator_pattern = r'@app\.(get|post|put|delete|patch)\s*\(\s*["\']([^"\']+)["\']'
    ...
    def generate_markdown_sections(self) -> Dict[str, str]:
        ...
        if self.nodes:
            ...
            for i, node in enumerate(self.nodes, 1):
                nodes_md += f"{i}. **{node}_node**: {node.description or f'Nodo de {node}'}\n"
            nodes_md += "\n"
        ...
        if self.endpoints:
            ...
            for endpoint in self.endpoints:
                endpoints_md += f"- `{endpoint.method} {endpoint.path}` - {endpoint.description}\n"
            endpoints_md += "\n"
